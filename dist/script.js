...
async function Draw(){
  ...
  if(!t){
    ...
    br = 2
    sp = 1
    grid = Array(rw*cl*br).fill().map((v,i)=>{
      X = ((i%cl)-cl/2+.5)*sp
  ...
  oX=0, oY=0, oZ=20
  Rl=C(t/4)/2, Pt=-.2+S(t/3)/8, Yw = S(t/2)/2
  
  if(t&&!((t*60|0)%sFreq) && !started){
    idx = grid.length*Rn()|0
    X = grid[idx][0]
    Y = grid[idx][1]
    Z = grid[idx][2]
    spawnSparks(X,Y,Z)
    ...
    if(v[3]>0){
      ...
      sparks.map(q=>{
        if(q[6]){
          ...
          Y2 = q[1]
          Z2 = q[2]
          if(Math.hypot(X2-X1,Y2-Y1,Z2-Z1)<sp/2){
            v[3] = 0
            //q[6] = 0
            spawnSparks(X1,Y1,Z1)
          }
        }
      })
      if(v[3]){
